// users.js — versión personalizada para AIOban
window.API_BASE = "https://aioban-api.compagnucci-ja.workers.dev";
(() => {
  const ENDPOINT = (window.API_BASE || "").replace(/\/+$/,"");
  ...
  function addNavButtons() {
    ...
    btnSignup.addEventListener("click", async () => {
      ...
      const res = await api("/api/auth/signup", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, password, username }) });
      const data = await res.json().catch(()=>({}));
      alert(res.ok ? "Cuenta creada. Revisá tu email si pide confirmación." : ("Error: " + (data.error||res.status)));
    });
